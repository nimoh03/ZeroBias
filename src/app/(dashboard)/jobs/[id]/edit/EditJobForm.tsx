...
import { ArrowLeft, Briefcase, Target, CheckCircle2, Loader2, X, FileText, Trash2, Calendar, AlertTriangle } from "lucide-react";
...
export default function EditJobForm({
  ...
  // Setup schedule parsing (if a previous finalAction existed, default to single_link for easy editing)
  const hasExistingAction = !!initialData.finalAction;
  const [scheduleInterview, setScheduleInterview] = useState(initialData.scheduleInterview ?? hasExistingAction);
  const [schedulingType, setSchedulingType] = useState<'single_link' | 'multiple_links' | 'time_slots'>('single_link');
  const [singleLink, setSingleLink] = useState(initialData.finalAction || "");
