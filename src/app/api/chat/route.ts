...
const cvSection = candidateRow?.cv_summary
  ? `\n\nCV ALREADY ON FILE (verified from an uploaded resume — treat these as confirmed facts). Before asking ANY screening question, check whether the CV summary below already answers it. If it does, do NOT ask that question at all — treat it as confirmed and move straight to the next thing you still need. This is the entire reason the CV was collected before screening began: to skip redundant questions, not to re-ask them and just accept a matching answer afterward. Only ask about something if the CV summary is silent on it, or only vaguely implies it (e.g. gives total years of experience but not the specific skill you need to confirm). If the candidate later states something related anyway (unprompted), actively cross-check it against the CV, especially anything tied to a dealbreaker (school attended, years of experience, past employer, certifications, etc) — if it's consistent, silently accept it; only flag it as a concern if what they say directly contradicts the CV. If the name on the CV clearly differs from the name the candidate gave you (not just a minor formatting difference like a missing middle name), do not silently accept it — treat it as a concern and note it plainly in your eventual concerns list, without letting it override an otherwise-passing match on its own.\n${candidateRow.cv_summary}`
  : candidateRow?.cv_url
