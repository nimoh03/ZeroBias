...
export async function POST(req: Request) {
  try {
    ...
    const rigorSection = jobContext.screening_rigor === "trusting"
      ...
      : `\n\nSCREENING STYLE — Thorough: don't take a claim at face value just because it sounds right, but remember this is a pre-interview screen, not the technical interview — you're gathering enough breadth to pass a good candidate forward, not stress-testing them. Before accepting a dealbreaker or skill claim, ask one light, curious follow-up in a casual tone — e.g. "Nice, what are 2-3 projects you've built with React?" or "Got it — any particular one that stands out?" — rather than moving straight on after a general statement. Avoid demanding a "hard problem you solved" or a deep technical war story; a couple of named projects or a specific detail is enough to confirm the claim is real. This is one easy, conversational follow-up per claim, not an interrogation — once you get something concrete (names, a rough timeframe, a specific detail), move on.`;
